...
try:
    from data.utils.process import generate_trimoe_synthetic_data
except ImportError:
    def generate_trimoe_synthetic_data(*_args, **_kwargs):
        raise NotImplementedError("trimoe_synthetic data generation is not included in this anonymous release.")
...
def main(args):
    ...
    elif args.dataset == "trimoe_synthetic":
        dataset = generate_trimoe_synthetic_data(args, partition, stats)
    ...
    if args.dataset not in ["femnist", "celeba"]:
        if args.split == "sample":
            ...
            for client_id in partition["separation"]["test"]:
                ...
                np.random.shuffle(indices)
                # testset_size = int(len(indices) * args.test_ratio)
                testset_size = int(len(indices) * 0.9)
                valset_size = int(len(indices) * args.val_ratio)
                trainset, valset, testset = (
                    indices[testset_size + valset_size :],
                    indices[testset_size : testset_size + valset_size],
...
if __name__ == "__main__":
    ...
    # )
    # parser.add_argument(
    #     "-d", "--dataset", type=str, choices=DATASETS.keys(), default="trimoe_synthetic"
    # )

    parser.add_argument(
        "-d", "--dataset", type=str, choices=DATASETS.keys(), default="cifar100"
    )

    parser.add_argument("--iid", type=float, default=0.0)
    parser.add_argument("-cn", "--client_num", type=int, default=50)
    # parser.add_argument("-cn", "--client_num", type=int, default=300)
    ...
    # )
    parser.add_argument(
        "-sp", "--split", type=str, choices=["sample", "user"], default= "user"
    )
    parser.add_argument("-vr", "--val_ratio", type=float, default=0.0)
    parser.add_argument("-tr", "--test_ratio", type=float, default=0.25)
    # parser.add_argument("-tr", "--test_ratio", type=float, default=0.25)
    # parser.add_argument("-tr", "--test_user_ratio", type=float, default=0.9)
    parser.add_argument("-pd", "--plot_distribution", type=int, default=1)

    # Randomly assign classes
    parser.add_argument("-c", "--classes", type=int, default=100)
    # parser.add_argument("-c", "--classes", type=int, default=40)
    # parser.add_argument("-c", "--classes", type=int, default=60)
    # parser.add_argument("-c", "--classes", type=int, default=80)
    # parser.add_argument("-c", "--classes", type=int, default=100)
    # parser.add_argument("-c", "--classes", type=int, default=100)
    ...
    parser.add_argument("-a", "--alpha", type=float, default=0.3)
    parser.add_argument("-ms", "--min_samples_per_client", type=int, default=100)
    ...
    # For trimoe_synthetic data only
    ...
    # parser.add_argument("--ood_domains", nargs="+", default="sketch")
